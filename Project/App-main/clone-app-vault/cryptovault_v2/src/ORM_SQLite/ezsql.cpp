...
#else
        int rs = SQLITE_OK;
        ezsqlWarning << "Standard SQLite detected - encryption functions not available";
#endif
        if (rs == SQLITE_OK) {
            ...
            if (mesgError) sqlite3_free(mesgError);
        }
    }
...
#else
            ezsqlWarning << "Standard SQLite detected - rekey not supported";
            return true;
#endif
        }
    }
