...
ColumnLayout {
    ...
    ScrollView {
        ...
        Item {
            ...
            // Đảm bảo vùng nhận click nền luôn phủ kín ít nhất là hết chiều cao ScrollView
            height: Math.max(vaultScrollView.availableHeight, vaultListLayout.implicitHeight)

            // MouseArea Nền: Bắt click vào vùng trống
            MouseArea {
                anchors.fill: parent
                onClicked: {
                    // Khi click vào vùng trống, gửi tín hiệu -1 lên SideBar để bỏ chọn
                    vaultSectionRoot.vaultClicked(-1)
                }
            }
            ColumnLayout {
                id: vaultListLayout
                ...
                Repeater {
                    ...
                    delegate: NavDelegate {
                        ...
                        onVaultDoubleClicked: function(index) {
                            vaultSectionRoot.vaultDoubleClicked(index)
                        }
                        // THÊM DÒNG NÀY ĐỂ BẮT TÍN HIỆU ĐỔI PASSWORD:
                        onChangeVaultPasswordRequested: function(index) {
                            vaultSectionRoot.changePasswordRequested(index)
                        }
                    }
                }
            }
