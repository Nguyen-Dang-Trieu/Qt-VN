...
// INSERT
bool CPathDB::saveVaultFolder(const QString &name, const QString &nameDB, const QString &path) {
...
//    QByteArray KEK = scrypt(HARDCODED_PATH_DB_PASSWORD, dbSalt, n_crypt, r_crypt, p_crypt, KEY_LEN);
    ...
    // add start 20260415
    QByteArray KEK = 0;
    if (!m_usernameSignup.isEmpty() && !m_passwordSignup.isEmpty()) {
        QByteArray userSalt = m_passwordSignup.toUtf8();
        KEK = scrypt(m_usernameSignup, userSalt, n_crypt, r_crypt, p_crypt, KEY_LEN);
        m_derivedKey = KEK.toHex();
    }
    // add start 20260415
...
// add start 20260415
void CPathDB::setUsernameSignup(const QString &username) {
    m_usernameSignup = username;
}

void CPathDB::setPasswordSignup(const QString &password) {
    m_passwordSignup = password;
}

QString CPathDB::getUsernameSignup() const {
    return m_usernameSignup;
}

QString CPathDB::getPasswordSignup() const {
    return m_passwordSignup;
}

QString CPathDB::getKEK() const {
    return m_derivedKey;
}
// add end 20260415

// add start 20260420
bool CPathDB::openWithKEK(const QString& dbPath, const QString& kekHex) {
    currentDBPath = dbPath;

    if (!QFile::exists(dbPath)) {
        LOG_ERROR("Account Database does not exist: " + dbPath);
        return false;
    }
    ...
    if (_db_.open(params)) {
        // Đọc thử bảng để ép SQLCipher giải mã, kiểm tra KEK có thực sự đúng không
        _db_.createTable<CSavedVault>();

        // Lưu KEK hợp lệ vào bộ nhớ tạm
        m_derivedKey = kekHex.toUtf8();
        return true;
    }

    LOG_ERROR("Mở PathDB bằng KEK thất bại (KEK sai hoặc file hỏng).");
    return false;
}

bool CPathDB::changePassword(const QString& newPassword) {
    if (_db_.connection() == nullptr) {
        LOG_ERROR("Cannot change password: PathDB is not open!");
        return false;
    }
    m_passwordSignup = newPassword;
    QString newKeyDB = deriveDbKey(currentDBPath);
    ...
    // Chạy lệnh PRAGMA rekey của SQLCipher
    ...
    LOG_INFO("Account Password changed (Rekey) successfully!");
    return true;
}
// add end 20260420

/* ---------- Operations with Table User_InfoVaults --- */
QString CPathDB::getNameDB(const QString &name) {
    if (!_db_.connection()) return "";
